...
export function addGeofences(map, spv = '') {
  const BASE_SIZE   = 32;
  const BASE_ZOOM   = 12;
  const SCALE_STEP  = 5;
  const MAX_SIZE    = 64;
  ...
  // Only add ULTRATECH-specific hardcoded geofences for the ULTRATECH project
  if (!spv || spv === 'ULTRATECH') {
    // ── CIRCLE GEOFENCES ──────────────────────────────────────────────
  ...
  // Return cleanup function to remove all added layers
